...
\begin{small}
    ...
    \cbox{red}{
        The database may crash at any time (e.g., someone pulls the power plug). After a crash, the system's \blue{recovery module} must ensure
        \myitems{
            \item the changes made by the committed transactions are retained, and
            \item those made by the others transactions are canceled.
        }
    }
...
%-------------------------------------------------------------
\myfrm{
    \xmybox{Multiple Copies of the Same Value}

    \cbox{blue}{
    For every value in a database, there can be multiple copies:
    \myitems{
        \item one disk copy;
        \item one (\blue{global}) memory copy of the system;
        \item one (\blue{local}) memory copy \bred{per} transaction.
    }
    }
...
\myfrm{
    \cbox{green}{
        \blue{Example 1:} Consider the schedule below \bred{at the transaction level}.
...
\myfrm{
    \cbox{green}{
        ...
        \cbox{red}{
            What should we do if a crash happens after step 5? How about after step 8? Step 9?
        }
        \end{center}
    }
}
...
\myfrm{
    ...
    Logging is a wildly-adopted approach to implement the recovery mechanism in database systems.

    \vgap

    \cbox{blue}{
    A \blue{log file} is a special file on the disk maintained by the system, containing a sequence of \blue{log records} that capture transactions' behavior.
    }
...
\myfrm{
    ...
    We will introduce three logging methods --- all in the so-called \blue{write-ahead logging} category --- which use different sets of log records. However, two records are \bred{common to all methods}.

    \myitems{
        \item \red{\dblog{START $T$}}: Indicating the start of transaction $T$. This must be the first log record of $T$.
        \item \red{\dblog{COMMIT $T$}}: Indicating that transaction $T$ has committed.
        %\item \red{\dblog{ABORT $T$}}: Records that transaction $T$ has aborted.
    }

    \cbox{blue}{
        \blue{Write-Ahead Logging:} Before modifying the disk content, write a necessary log about the modification to the disk.
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    ...
    \myitems{
        ...
        \blue{Meaning:} transaction $\red{T}$ will change a tuple's attribute $\red{A}$, and the global memory copy of the attribute has value $\red{a_{old}}$ currently.
    }
}
%-------------------------------------------------------------
\myfrm{
    ...
    \dblog{$T, A, a_{old}$} $\Rightarrow$ changing the disk copy of $A$ $\Rightarrow$ \dblog{COMMIT $T$}
}
%-------------------------------------------------------------
\myfrm{
    \cbox{green}{
...
%-------------------------------------------------------------
\myfrm{
    \xmybox{Recovery from a System Crash}

    \vgap

    Suppose that a crash happens.
    %\vgap

    \cbox{blue}{
        The recovery module starts by identifying the transactions that were committed and those that were running at the moment of crash, respectively.
    }
    \blue{Think:} How?
...
\myfrm{
    \cbox{green}{
        ...
        The recovery algorithm processes the log bottom up and finds no disk modifications necessary.
    }
}
%-------------------------------------------------------------
\myfrm{
    \cbox{green}{
        ...
        The recovery algorithm processes the log bottom up and sets the disk copy of $A$ to 10.
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    ...
    \myitems{
        ...
        \blue{Meaning:} transaction $\red{T}$ will change a tuple's attribute $\red{A}$ to $\red{a_{new}}$.
    }
}
%-------------------------------------------------------------
\myfrm{
    ...
    Intuitively, the ``correct'' ordering is: \\
    \dblog{$T, A, a_{new}$} $\Rightarrow$ \dblog{COMMIT $T$} $\Rightarrow$ changing the disk copy of $A$
}
%-------------------------------------------------------------
\myfrm{
    \cbox{green}{
        ...
        \begin{tabular}{c|c|c|c|c|c}
            ...
            5& $T_1: \cmt$ & 10 & 11 & n/a & \dblog{COMMIT $T_1$} \\
            6& $\out(A)$ & 11 & 11 & n/a &
        \end{tabular}
...
\myfrm{
    \cbox{green}{
        ...
        The recovery algorithm processes the log top down and
        sets the disk copy of $A$ to 11.
    }
}
%-------------------------------------------------------------
\myfrm{
    \cbox{green}{
        ...
        The recovery algorithm processes the log top down and finds no disk modifications necessary.
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    ...
    \myitems{
        ...
        \blue{Meaning:} transaction $\red{T}$ will change a tuple's attribute $\red{A}$ to $\red{a_{new}}$, and the global memory copy of the attribute has value $\red{a_{old}}$ currently.
    }
}
%-------------------------------------------------------------
\myfrm{
    ...
    Intuitively, the ``correct'' ordering is: \\
    \dblog{$T, A, a_{old}, a_{new}$} $\Rightarrow$ \dblog{COMMIT $T$} \\
    \dblog{$T, A, a_{old}, a_{new}$} $\Rightarrow$ changing the disk copy of $A$ \\
    Arbitrary ordering between \dblog{COMMIT $T$} and changing the disk copy of $A$.
}
%-------------------------------------------------------------
\myfrm{
    \cbox{green}{
...
\myfrm{
    \cbox{green}{
        ...
        In the redo phase, the recovery algorithm processes the log top down and sets the disk copy of $A$ to 11.

        \vgap

        The undo phase processes the log bottom up and finds no disk modifications necessary.
    }
}
%-------------------------------------------------------------
\myfrm{
    \cbox{green}{
        ...
        The redo phase processes the log top down and finds no disk modifications necessary.

        \vgap

        The undo phase processes the log bottom up and
        sets the disk copy of $A$ to 10.
    }
}
%-------------------------------------------------------------
\myfrm{
    \cbox{yellow}{
        \blue{Remark:} Ensuring the correctness of recovery control may require applying an appropriate concurrency control protocol.
    }
...
\myfrm{
    \cbox{green}{
        \blue{Example 5:} Suppose that \bred{undo/redo logging} is used, and the system sees the following log after a crash.
        \begin{center}
        \begin{tabular}{c}
            log \\
            \hline
        ...
        What would be the value of $A$ after recovery? Does it make sense?

        \cbox{red}{
            Can this log be created if a concurrency control protocol taught in this course is applied?
        }
    }
}
%-------------------------------------------------------------
\myfrm{
    \cbox{yellow}{
        \blue{Remark:} Our discussion has focused on \bred{system crashes}.

        \vgap

        In practice, a transaction may decide to abort itself, in which case a \blue{rollback} process is needed to cancel its modifications to the database. How would you perform the rollback using the logging methods you have learned?
    }
}
%-------------------------------------------------------------
\end{document} 
