...
\myfrm{
    ...
    \cbox{blue}{
        \blue{Rule 2:} A log record \red{\dblog{$T, A, a_{old}$}} must be written to the disk \bred{before} the disk copy of $A$ can change.
    }

    \cbox{blue}{
        \blue{Rule 3:} The log record \red{\dblog{COMMIT, $T$}} is written to the disk \bred{after} all the changes of $T$ have been written to the disk.
    }
...
\myfrm{
    ...
    \cbox{blue}{
        \blue{Rule 2:} Every log record of the form \red{\dblog{$T, A, a_{new}$}} must be written to the disk \bred{before} \red{\dblog{COMMIT, $T$}} is written to the disk.
    }
