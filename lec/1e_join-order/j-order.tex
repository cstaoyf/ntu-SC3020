...
\myfrm{
    ...
    \cbox{red}{
        Size oracles do not exist. In reality, given any join, a database system can compute an \bred{estimate} of its output size. These estimates serve the purpose of size oracle. We will discuss such estimates in a tutorial.
    }
    ...
    \cbox{blue}{
        We will show that the problem can be solved with less than $\red{2^k}$ oracle calls by dynamic programming.
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{ \label{slide:thm}
    \cbox{blue}{
        ...
        If $|S| > 2$, then
        \myeqn{
            \best(S) = \size(S) + \min_{\substack{\text{subset $\red{S'} \subseteq S$}\\ \text{with $\red{|S'| = |S| - 1}$}}} \best(S'). \label{thm:2}
        }
    }
...
\myfrm{
    \cbox{blue}{
        \blue{Corollary 2:} Let $\red{S}$ be a subset of $\set{R_1, ..., R_k}$ with $|S| \ge 3$. If $\best(S')$ is available for every subset $S' \subseteq S$ with $|S'| = |S| - 1$, we can compute $\best(S)$ with a single oracle call.
    }
    ...
    \blue{Step $\red{t \ge 2}$:} For every $S \subseteq \set{R_1, ..., R_k}$ with $\red{|S| = t+1}$, compute $\best(S)$ with one oracle call.
    }

    \vgap

    We obtain the final answer $\best(\set{R_1, ..., R_k})$ after Step $k - 1$.
}
%-------------------------------------------------------------
\myfrm{
    \cbox{green}{
        ...
        \begin{tabular}{cc}
            ...
            \begin{minipage}{0.47\linewidth}
                \begin{tabular}{c|c|c}
                    ...
                    $R_2,R_4$ & 100 & - \\
                    $R_3,R_4$ & 30 & - \\
                    $R_1, R_2, R_3$ & 1020 & $R_2, R_3$ \\
                    $R_1, R_2, R_4$ & 1550 & $R_1, R_4$ \\
                    $R_1, R_3, R_4$ & 830 & $R_3, R_4$ \\
                    $R_2, R_3, R_4$ & 90 & $R_2, R_3$ \\
                    $R_1, R_2, R_3, R_4$ & 110 & $R_2, R_3, R_4$
                \end{tabular}
            \end{minipage}
        \end{tabular}
...
\myfrm{
    ...
    $\set{R_1, R_2, ..., R_k}$ has less than $\red{2^k}$ subsets of size at least 2. For every such subset $\red{S}$, the value $\best(S)$ is computed with one oracle call. The total number of oracle calls is therefore less than $2^k$.
    ...
    \cbox{red}{
        ...
        \blue{Hint:} Look at the ``reason'' column of the previous slide.
    }
}
%-------------------------------------------------------------
\end{document} 
