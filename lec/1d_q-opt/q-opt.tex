...
\myfrm{
    \begin{tabular}{cc}
        ...
        \begin{minipage}{0.7\linewidth}
            ...
            \myitems{
                \item $\red{B(R_1)} = \red{B(R_2)} = 10000$ \\[-1mm]
                \item $\sigma_{B = 1} R_1 $ has $1000$ blocks. $R_1$ is clustered on $B$ and there is a hash index on $R_1.B$. \\[-1mm]
...
\myfrm{
    \cbox{green}{
        ...
        The methods we have discussed are by no means the only methods implemented in existing database systems. The query optimization module of a database system is a highly sophisticated and often kept as a commercial secret.
    }
}
%-------------------------------------------------------------
\end{document} 
