...
\myfrm{
    ...
    \begin{tabular}{cc}
        ...
        \begin{minipage}{0.7\linewidth}
            ...
            \myitems{
                ...
                \item $\red{M} = 100$ (the memory has 100 blocks).
            }
        \end{minipage}
        &
    ...
    \begin{itemize}
        ...
        \item Compute $\red{R_4} = \sigma_{B = 1 \wedge C \in [100,200](R_3)}$ by reading $R_3$ and materialize $R_4$ \\
...
\myfrm{
    ...
    \begin{itemize}
        \item Compute $\red{R_3} = \sigma_{B=1} R_1$ by reading $R_1$ and materialize $R_3$ \\
        $\Rightarrow$ 10000 (reading) + 1000 (mat.) I/Os \\[-1mm]
        \item Compute $\red{R_4} = \sigma_{B=1 \wedge C \in [100, 200]} R_2$ by reading $R_2$ and materialize $R_4$ $\Rightarrow$ 10000 (reading) + 800 (mat.) I/Os \\[-1mm]
    ...
    \begin{tabular}{cc}
        ...
        \begin{minipage}{0.75\linewidth}
            ...
            \myitems{
                ...
                \item $R_2$ is sorted and clustered on $C$. \\
                There is a sparse B-tree on $R_2.C$ with 3 levels. \\ [-1mm]
                \item $\sigma_{B = 1 \wedge C \in [100, 200]} R_2$ has $800$ blocks  \\[-1mm]
                \item $\sigma_{B = 1 \wedge C \in [100,200]} R_1 \bowtie R_2$ has $\red{B_2}$ blocks. \\[-1mm]
                \item $\red{M} = 100$.
            }
...
\myfrm{
    \begin{tabular}{cc}
        ...
        \begin{minipage}{0.7\linewidth}
            ...
            \myitems{
                \item $\red{B(R_1)} = \red{B(R_2)} = 10000$ \\[-1mm]
                \item $\sigma_{B = 1} R_1 $ has $1000$ blocks. There is a hash index on $R_1.B$. \\[-1mm]
    ...
    \begin{itemize}
        ...
        \item \bred{As soon as getting a tuple} $\red{t} \in R_3$: probe the hash index on $R_1.B$ to find all tuples $\red{s} \in \sigma_{B=1} R_1$; output $(s.A, t.C)$. \\
...
\myfrm{
    ...
    \begin{itemize}
        \item Compute $\red{R_3} = \sigma_{B=1 \wedge C \in [100, 200]} R_2$ by reading $R_2$ $\Rightarrow$ 10000 I/Os. \\[-1mm]
        \item \bred{Every time we have accumulated 99 blocks of new tuples of $R_3$ in memory}, probe the hash index on $R_1.B$ to find $\red{R_4} = \sigma_{B=1} R_1$; output $(s.A, t.C)$ for each $t \in R_3$ in memory and every $s \in R_4$. \\
        $\Rightarrow$ $\lc 800/99 \rc \cdot 1000 = 9000$ I/Os.
    \end{itemize}
    In total: $19000$ I/Os.
}
%-------------------------------------------------------------
\myfrm{
    \cbox{green}{
        Query optimization is an art and is an active research area.

        \vgap

        The methods we have discussed are by no means the only methods implemented in existing database systems. In fact, the query optimization module of a database system is a highly sophisticated and often kept as a commercial secret.
    }
}
%-------------------------------------------------------------
\end{document} 
