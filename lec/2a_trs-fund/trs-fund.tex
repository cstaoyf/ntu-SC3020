...
\myfrm{
    ...
    \cbox{green}{
        ...
        \begin{tabular}{c|c}
            $\red{T}$ & remark\\
            \hline
            \rd($\red{A}$) & read a tuple's value into a memory copy \\
            $A = A + 1$ &\\
        ...
        Either the tuple's value increases 1 or it remains unchanged.
    }
}
%-------------------------------------------------------------
\myfrm{
    ...
    \cbox{green}{
        ...
        \begin{tabular}{c|c}
            ...
            \rd($\red{A}$) & \\
            $A = A + 1$ &\\
            \wt($A$) & \bred{suppose that an I/O error occurs here}
        \end{tabular}
        \end{center}
...
\myfrm{
    \cbox{blue}{
        A schedule is \blue{serial} if there is no interleaving among the instructions of different transactions.
    }
...
\myfrm{
    \cbox{blue}{
        A schedule is \blue{serializable} if it results in the same database state as \bred{some} serial schedule.
    }

    \cbox{green}{
        \blue{Example:} The following schedule is serial.
        \begin{center}
        \begin{tabular}{c|c}
            $\red{T_1}$ & $\red{T_2}$\\
            \hline
