...
\myfrm{
    ...
    I/O cost:
    \myitems{
        \item Number of Read I/Os $= \sum_{i=1}^{M-1} B(R_i)$\\
        (every input block read once)
        \item Number of Write I/Os $\le \sum_{i=1}^{M-1} B(R_i)$ \\
        (the sorted file cannot have more blocks).
    }

    \cbox{green}{
        Total I/O cost $\le 2 \sum_{i=1}^{M-1} B(R_i)$.
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    ...
    Recall that the input is $\red{R}$, which occupies $\red{B(R)}$ blocks.
    ...
    Chop $R$ into $\red{n_0} = \red{\lc {B(R)/M} \rc}$ \blue{runs}, each consisting of $M$ consecutive blocks (except possibly the last run).
    ...
    I/O cost = $2 \cdot B(R)$ (\blue{think:} why 2?)
}
%-------------------------------------------------------------
\myfrm{
    \xmybox{(Disk-Based) Sorting}
    ...
    I/O cost $\le 2 \cdot B(R)$ (\blue{think:} why?)
}
%-------------------------------------------------------------
\myfrm{
    \xmybox{(Disk-Based) Sorting}
