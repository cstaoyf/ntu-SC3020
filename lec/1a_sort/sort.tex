...
\myfrm{
    ...
    $\red{R} =$ a set of elements drawn from a total order. \\
...
\myfrm{
    ...
    $\red{M} =$ the number of memory blocks (a.k.a., the buffer blocks). \\
    We have sets $\red{R_1}, \red{R_2}, ..., \red{R_{M-1}}$ where
    \myitems{
        \item all the elements in $R_1, ..., R_{M-1}$ are drawn from a total order;
...
\myfrm{
    ...
    Whenever an input buffer is empty --- say the buffer is for $\red{R_i}$ --- fill it with the next block of $R_i$ (if it exists).
...
\myfrm{
    ...
    I/O cost = $2 \cdot B$ (\blue{think:} why?).
}
%-------------------------------------------------------------
\myfrm{
    \xmybox{(Disk-Based) Sorting}
    ...
    I/O cost $\le 2 \cdot B$ (\blue{think:} why 2? \blue{hint:} apply our merging algorithm).
}
%-------------------------------------------------------------
\myfrm{
    \xmybox{(Disk-Based) Sorting}
...
\myfrm{
    ...
    \cbox{red}{
        ...
        \blue{Hint:} no need to change the algorithm --- what would be a clever ``total order'' here?
    }
