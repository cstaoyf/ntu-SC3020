...
\myfrm{
    ...
    \bred{If $x_1 = x_2$}, read into memory all tuples of $R_1$ with $X$-values equal to $x_1$. \\
...
\myfrm{
    ...
    \cbox{blue}{
        \blue{Remark:} The above analysis has not considered the cost of writing the join result to the disk. If that is necessary, you should add $\red{B_\mit{out}}$ to the I/O cost, where $B_\mit{out}$ is the number of blocks needed to store the join result, provided that you have an additional memory block to serve as the output buffer.
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    \cbox{yellow}{
        Returning to the binary join problem $\red{R_1(X, Y) \bowtie R_2(X, Z)}$, next we outline a refinement of the sort join algorithm that can reduce the I/O cost to $\red{3 (B_1 + B_2)}$ when the memory size (i.e., $M$) is reasonably large.
...
\myfrm{
    ...
    \myitems{
        \item $n_1 + n_2 < M$;
        \item (\blue{skew condition modified}) for any $X$-value, the tuples of $R_1$ having that $X$-value fit in at most $\red{M - n_1 - n_2}$ blocks.
    }
    ...
    (\blue{hint}: how did you solve the 3-star join problem?).
