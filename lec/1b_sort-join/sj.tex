...
\myfrm{
    ...
    \cbox{yellow}{
        ...
        $\red{R_3(X, A_3)}$: A relation with attributes $X$ and $A_2$. \\
        $\red{B_i} =$ the number of disk blocks that $R_i$ occupies ($1 \le i \le 3$). \\
        $\red{M} =$ the number of memory blocks (a.k.a., the buffer blocks).

        \vgap

        Under what ``skew-free condition'' would you be able to modify the sort-join algorithm to compute $R_1 \bowtie R_2 \bowtie R_3$ in $5 (B_1 + B_2 + B_3)$ I/Os, assuming $B_i \le M(M-1)$ for all $i \in [1, 3]$?
    }
}
%-------------------------------------------------------------
\end{document} 
