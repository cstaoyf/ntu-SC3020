...
\begin{frame}
\begin{small}
    Recall that the \bred{serializable} isolation level should be enforced for critical transactions (such as money transfers). This lecture will discuss how to do so with \bred{locking}.
\end{small}    
\end{frame}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    ...
    \cbox{blue}{
        \blue{Rule 2:} Before a transaction can \bred{read} a database value $\red{A}$, it must be holding a lock (of any type) on $A$. Before a transaction can \bred{write} a database value $\red{A}$, it must be holding an \bred{X-lock} on $A$.
    }
...
\myfrm{
    ...
    \cbox{blue}{
        \blue{Rule 5:} If a transaction is holding a U-lock on a value $\red{A}$, it can issue an \blue{upgrade request} to acquire an X-clock on $A$. The database grants the request only if \bred{no other locks exist on $A$}.
...
\myfrm{
    ...
    Granted: Rule 5. $T_2$ now holds an X-lock on $A$.
...
\myfrm{
    ...
    Granted: Rule 4. $T_1$ holds no more locks on $A$.
...
\myfrm{
    ...
    Granted: Rule 3. $T_2$ now holds an S-lock on $A$.
