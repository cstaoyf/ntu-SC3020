...
\myfrm{
    ...
    \myitems{
        \item We assume that $M - 1$ is a multiple of 3 at least 6 (hence, $M \ge 7$).
    }
...
\myfrm{
    ...
    Let $\red{\mathbb{D}}$ be the domain of the attribute $X$. Set
    \myeqn{
        \red{U} = (M-1)/3 \nn
    }
...
\myfrm{
    ...
    \cbox{blue}{
        \blue{Remark:} Each bucket can have \bred{at most one non-full block}, and this block \bred{must reside in memory}. We cannot afford to write those blocks to disk; otherwise, the I/O cost can increase to $2B_1 + U$ in the worst case (\blue{think:} why?).
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    ...
    For each $\red{h} \in [1, U]$, apply BNL to compute $R_1(h) \bowtie R_2(h)$. By the good hashing assumption, $R_1(h)$ can be loaded into $U$ memory blocks. Then, use one memory block to scan $R_2(h)$.
    \myitems{
...
\myfrm{
    ...
    \cbox{red}{
        \blue{Remark.} Recall that our analysis requires the good hash uniform assumption. \bred{This assumption may not hold when data are ``skewed''.} For example, if all the tuples of $R_1(X, Y)$ share the same $X$-value, no hash function can satisfy the assumption as long as $B_1 > U$. In the presence of data skew, hash join can be prohibitively expensive.
    }
}
%-------------------------------------------------------------
