...
\begin{small}
    ...
    The protocol we will discuss is a simplified version of what is implemented in PostgreSQL.
\end{small}    
\end{frame}
%-------------------------------------------------------------
\myfrm{
    ...
    The assumption allows us to focus on the core ideas of MVCC.
}
%-------------------------------------------------------------
\myfrm{
    \cbox{blue}{
...
\myfrm{
    ...
    \cbox{green}{
        ...
        \blue{Invariant:} The transactions with IDs $r_1.\idins$, $r_2.\idins$, ..., $r_{\ell-1}.\idins$ must have committed. The transaction with ID $r_\ell.\idins$ is either running or has committed (i.e., it cannot be an aborted transaction).
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    ...
    \cbox{blue}{
        ...
        \myitems{
            ...
            \myitems{
                \item If $r_\ell.\idins = ID_T$, then set $r_\ell.v = v_{new}$.
                \item Set $r_\ell.\iddel = ID_T$ and append a record $(v, ID_T, \ttt{NULL})$ to the history.
            }
...
\myfrm{
    \cbox{blue}{
        ...
        It should be noted that multi-versioning is useful not only for concurrency control, e.g., it allows us to ``travel back in time'' to obtain the database content at any past timestamp. Multi-versioning is implemented in many database systems nowadays.
    }
}
%-------------------------------------------------------------
\end{document} 
