...
\myfrm{
    ...
    The assumption allows us to simplify the discussion and concentrate on the core principles behind MVCC's support for ``read committed''. It is not difficult to remove the assumption once those principles are clear, as briefly explained at the end of this lecture.
...
\myfrm{
    ...
    \cbox{red}{
        \bred{Remark:} PostgreSQL has the requirement that a transaction $\red{T_1}$ starts before another transaction $\red{T_2}$, the ID of $T_1$ is smaller than that of $T_2$. We do not need this requirement today.
    }
}
%-------------------------------------------------------------
\myfrm{
...
\myfrm{
    ...
    \cbox{green}{
        ...
        If transaction 110 modifies $t$ to $(x_4, y_4)$, the log becomes
            \begin{center}
             (($x_1, y_1$), 100, 103) \\
             (($x_2, y_2$), 103, 110) \\
             (($x_4, y_4$), 110, \ttt{NULL}).
...
\myfrm{
    ...
    \cbox{green}{
        ...
        Suppose that transaction 111 modifies $t$ to $(x_4, y_4)$ after transaction 110 has committed. The log becomes
            \begin{center}
             (($x_1, y_1$), 100, 103) \\
             (($x_2, y_2$), 103, 110) \\
             (($x_3, y_3$), 110, 111) \\
...
\myfrm{
    ...
    \cbox{green}{
        ...
        If transaction 110 aborts, the log becomes
            \begin{center}
             (($x_1, y_1$), 100, 103) \\
             (($x_2, y_2$), 103, \ttt{NULL}) .
        \end{center}
...
\myfrm{
    ...
    \cbox{green}{
        ...
        \myitems{
            ...
            \item If transaction 109 issues $\rd(t)$ with transaction 111 already committed, it gets nothing, i.e., the \bred{last committed version}.

            \item If transaction 109 issues $\rd(t)$ with transaction 111 still running, it gets $(x_2, y_2)$, i.e., the \bred{last committed version}.
        }
    }
}
%-------------------------------------------------------------
