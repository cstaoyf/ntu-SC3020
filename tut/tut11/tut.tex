...
\myitems{
    ...
    \item [(c)] Is the schedule conflict serializable?
}
...
\extraspacing {\bf Problem 5.} Consider the (system-level) schedule below:
...
\noindent Assume that the initial values of $A$, $B$, and $C$ are all 10. Answer the following questions:
\myitems{
    \item [(a)] Give the log created by undo/redo logging.
    \item [(b)] Suppose that a power outage occurs right before instruction 12. What are the values of $A$, $B$, and $C$ on the disk?
}
...
\extraspacing {\bf Problem 6.} Suppose that the system finds the following log after a reboot from crash.
...
\myitems{
    \item [(a)] When the crash happened, what were the states of transactions $T_1$, $T_2$, and $T_3$, respectively?

    \item [(b)] Describe how the recovery procedure will modify the disk values of $A$, $B$, and $C$.
}

\begin{sol}
\extraspacing {\bf Solution.} (a) $T_1$ and $T_3$ were committed, and $T_2$ was still running.

(b) The recovery procedure first performs a redo phase, which sets the disk values of $A$ and $C$ to 60 and 250, respectively. Then, the procedure performs an undo phase, which sets the disk values of $A$ and $B$ to 60 and 100, respectively.
\end{sol}
...
\extraspacing {\bf Problem 7.} Can MVCC produce a non-recoverable schedule?

\extraspacing {\bf Problem 8.} In Problem 6(b), what if another crash occurs {\em during} the recovery procedure? What do you think the system should do after a reboot?

\extraspacing {\bf Problem 9.} Argue that the MVCC protocol cannot produce the log below:
\begin{center}
    \begin{tabular}{l}
    \texttt{<START T1>} \\
    \texttt{<START T2>} \\
...
\extraspacing {\bf Problem 10.} Consider the (system-level) schedule below:
...
\noindent Discuss the rollback process at Step 12.
