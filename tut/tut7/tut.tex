...
Let $R_1(A, B)$ be a relation with attributes $A$ and $B$, and $R_2(A, C)$ be a relation with attributes $A$ and $C$. We assume that there is a B-tree on $R_2$ indexing the attribute $A$. Consider the following 4 algorithms for computing $R_1 \bowtie R_2$.
...
\myitems{
    ...
    \item The no skew assumption holds for SJ, and the good hashing assumption both holds for HJ.
