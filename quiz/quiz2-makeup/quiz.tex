...
\mytab{
    ...
    \ttt{where} \ttt{exists} ( \\
    \> \ttt{select} * \ttt{from} $R_2, R_3$ \\
    \> \ttt{where} $R_1.B = R_2.B$ \ttt{and} $R_2.C = R_3.C$ \ttt{and} $R_3.D \ge 100$ \\
    ) \\
    \ttt{group by} $A$
...
\begin{sol}
    \extraspacing {\bf Solution.} By Fact 6, $R_3'$ has $10^5$ tuples and fits in $10^4$ blocks. Hence, Step 1 requires $10^5 + 10^4 = 110,000$ I/Os.

    \vgap

    For each tuple $t_1 \in R_1$, Step 2.1 performs $1 \text{ (for probing the hash index)} + 100/10 \text{ (for reading the}$ $\text{ tuples of $R'_2(t_1)$)} = 11$ I/Os, while Step 2.2 performs $10^4$ I/Os. Hence, Step 2 performs in total $10^4 \times (11 + 10^4) = 100,110,000$ I/Os.
...
\begin{sol}
    ...
    Use HJ to compute $R_4 = R_3' \bowtie R_2$ in $3 \times (10^7 + 10^4)= 30,030,000$ I/Os. Note that $|R_4| \le |R_3'| = 10^5$. Hence, materializing $R_4$ takes another $10^4$ I/Os at most.

    \vgap

    Now, compute $R_5 = R_1 \bowtie R_4$ using HJ and materialize $\Pi_{A,B}(R_5)$. The HJ requires $3 \times (10^3 + 10^4) = 33,000$ I/Os, and the materialization takes at most $10^3$ I/Os.

    \vgap

    Finally, perform Step 4 in Question 1 in $4 \times 10^3$ I/Os.
